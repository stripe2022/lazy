// service-worker.js — Blindado (Lazy Barylie)
...
const ASSETS = [
  ...
  '/lazy/app.js',
  '/lazy/manifest.json',
  '/lazy/icons/icon-192.png',
  '/lazy/icons/icon-512.png',
  // Si usas libs, descomenta/ajusta:
  // '/lazy/libs/jspdf.umd.min.js',
  // '/lazy/libs/jspdf.plugin.autotable.min.js'
];
...
self.addEventListener('fetch', event => {
  ...
  if (req.mode === 'navigate') {
    event.respondWith(
      caches.match('/lazy/index.html').then(res => {
        if (res) return res;
  ...
  // Si está en la lista de ASSETS → cache-only (con fallback a red + guardar)
  ...
  // Para otros recursos (ej. imágenes) → cache-first
  event.respondWith(
    caches.match(req).then(res => {
